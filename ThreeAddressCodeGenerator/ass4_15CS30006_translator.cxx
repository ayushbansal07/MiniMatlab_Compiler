...
void symEntry::print(){
	...
	if(this->wasInitialised){
		switch(this->type->bType){
			...
			case(type_DOUBLE):	cout<<(this->init.doubleVal);		break;
			case(type_MATRIX):
			{
				//printf("{");
				cout.width(0);
				cout<<"{";
				vector<double> v = *(init.matVal);
				int height = this->type->leftIndex;
				int width = this->type->rightIndex;
				for(int i=0;i<height;i++){
					for(int j=0;j<width-1;j++){
						cout.width(0);
						cout<<fixed<<setprecision(3)<<left<<v[i*width + j]<<",";
						//printf("%3.3lf,",v[i*width + j]);
					}
					//printf("%3.3lf;",v[i*width + width -1]);
					cout.width(0);
					cout<<left<<fixed<<setprecision(3)<<left<<v[i*width + width -1]<<";";
				}
				//printf("}");
				cout.width(60);
				cout<<left<<"}";
...
int main() {
    ...
    //currentSymTab->print();
     ...
     //FILE *fp;
     //fp = fopen("ass4_15CS30006_quads3.out","w");
