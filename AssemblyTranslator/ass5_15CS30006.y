...
primary_expr:		IDENTIFIER 								 				{
					...
					|FLOATCONST 											{
																	              ...
																	              initialVal init; 
																	              init.doubleVal.d = $1;
																	              doubleLabels.push_back(init.doubleVal);
																	              init.dblLabel = doubleLabels.size()-1;
																	              currentSymTab->update($$.loc, init);
																	              $$.trueList = NULL;
              																	  $$.falseList = NULL;
																	              quad.emit(quadEntry(OP_COPY, $$.loc->name, $1));
...
postfix_expr:		primary_expr 											{
					...
					|postfix_expr OPENBRCSQ expression CLOSEBRCSQ OPENBRCSQ expression CLOSEBRCSQ			{
																												...
																												symEntry *temp3 = currentSymTab->genTemp(typeint);
																												symEntry *temp4 = currentSymTab->genTemp(typeint);
																												initialVal init; 
																									              init.intVal = SIZE_OF_INT*2;
																									              currentSymTab->update(temp2, init);
																									              quad.emit(quadEntry(OP_COPY, temp2->name, init.intVal));

																									              init.intVal = $1.type->rightIndex;
																									              currentSymTab->update(temp3, init);
																									              quad.emit(quadEntry(OP_COPY, temp3->name, init.intVal));

																									              
																									              init.intVal = SIZE_OF_DOUBLE;
																									              currentSymTab->update(temp4, init);
																									              quad.emit(quadEntry(OP_COPY, temp4->name, init.intVal));

																												//Calculating the offset (with extra 2*SIZE_OF_INT for storing array dimensions).
																							                    quad.emit(quadEntry(OP_MULT, temp->name, $3.loc->name, temp3->name));
																							                    quad.emit(quadEntry(OP_PLUS, temp->name, temp->name, $6.loc->name));
																							                    quad.emit(quadEntry(OP_MULT, temp->name, temp->name, temp4->name));
...
additive_expr:				multiplicative_expr 							{
							...
							|additive_expr PLUS multiplicative_expr    		{
																              ...
																              /*else{
																              	//Matrix Addition
																              	int sz = (typeop1->leftIndex)*(typeop1->rightIndex);
																              	for (int i=1;i<=sz;i++){
																              		//i*8
																              		symEntry * temp = currentSymTab->genTemp(new complexType(type_INT));
																              		initialVal init;
																              		init.intVal = i*8;
																              		currentSymTab->update(temp,init);
																              		quad.emit(quadEntry(OP_COPY,temp->name,i*8));
...
initializer:      assignment_expr       {
            ...
            |OPENBRCCUR initializer_row_list CLOSEBRCCUR      {
												            	...
												            	e.loc = quad.mat;
												            	e.type = e.loc->type;
												            	e.loc->init.matVal = new vector<dblunion>();
												            	vector<exp_t *> v = *($2);
												            	int sz = v.size();
												            	...
												            	//else{
												            		for(int i=0;i<sz;i++){
												            			...
												            			symEntry * temp2 = currentSymTab->genTemp(new complexType(type_INT));
																           			initialVal init;
																              		init.intVal = 2*SIZE_OF_INT + SIZE_OF_DOUBLE*i;
																              		currentSymTab->update(temp2,init);
																              		quad.emit(quadEntry(OP_COPY,temp2->name,init.intVal));
												            			quad.emit(quadEntry(OP_L_INDEX,quad.mat->name,temp2->name,temp->loc->name));
												            		}
